...
(async () => {
  try {
    ...
    for (let page = 1; page <= 64; page++) {
      ...
      $('.card').each((i, card) => {
        ...
        const company = {
          ...
          instrument: $button.attr('data-instrument') || null, // Инструмент поддержки
          support_program: $button.attr('data-support_program') || null, // Программа поддержки
          oked: $button.attr('data-oked') || null, // ОКЭД
        };
    ...
    const headersMap = {
      ...
      instagram: 'Instagram',
      website: 'Сайт',
      instrument: 'Инструмент поддержки',
